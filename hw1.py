...
c+=u'  <view class="navbar" style="padding-top:{{statusBarHeight}}px">\n'
c+=u'    <view class="navbar-left"><text class="navbar-title">\u65e5\u5e38\u8bb0\u5f55</text></view>\n'
c+=u'    <view class="navbar-right"><view class="navbar-icon" bindtap="switchPet"><text class="icon">\U0001f43e</text><text class="pet-name" wx:if="{{currentPet}}">{{currentPet.name}}</text><text class="pet-name" wx:else>\u9009\u62e9\u5ba0\u7269</text></view></view>\n'
c+=u'  </view>\n'
c+=u'  <scroll-view class="main-content" scroll-y>\n'
# date section
c+=u'    <view class="date-section">\n'
c+=u'      <view class="calendar-header">\n'
c+=u"        <view class=\"cal-nav-btn\" bindtap=\"prevMonth\"><text>\u2039</text></view>\n"
c+=u'        <text class="cal-month-title">{{currentMonth}}</text>\n'
c+=u"        <view class=\"cal-nav-btn\" bindtap=\"nextMonth\"><text>\u203a</text></view>\n"
c+=u'      </view>\n'
# weekday header
c+=u'      <view class="cal-weekdays">\n'
for w in [u'\u65e5',u'\u4e00',u'\u4e8c',u'\u4e09',u'\u56db',u'\u4e94',u'\u516d']:
    c+=u'        <view class="cal-weekday"><text>%s</text></view>\n'%w
c+=u'      </view>\n'
# calendar grid
c+=u'      <view class="cal-grid">\n'
c+=u"        <view class=\"cal-cell {{item.isOtherMonth?'other':''}}\ {{item.isSelected?'selected':''}}\ {{item.isToday?'today':''}}\ {{item.hasRecord?'has-record':''}}\" wx:for=\"{{calendarDays}}\" wx:key=\"index\" data-date=\"{{item.date}}\" bindtap=\"selectDate\">\n"
c+=u'          <text class="cal-day-num">{{item.day}}</text>\n'
c+=u'          <view class="cal-dot" wx:if="{{item.hasRecord&&!item.isOtherMonth}}"></view>\n'
c+=u'        </view>\n'
c+=u'      </view>\n'
c+=u'      <view class="selected-date-bar"><text class="selected-date-text">{{currentDate}}</text></view>\n'
c+=u'    </view>\n'
# records section
